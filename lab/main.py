...
class ServoController:
   ...
   def tick(self):
      ...
      if self.current_pos != self.target_pos:
         ...
         if (direction > 0 and delta + self.current_pos >= self.target_pos) or (direction < 0 and delta + self.current_pos <= self.target_pos):
...
def main():
   ...
   def i2c_request(args):
      bus.write_i2c_block_data(ADDR, 0, args)
   ...
   t = Thread(target=tick_servos, daemon=True)
   t.start()
